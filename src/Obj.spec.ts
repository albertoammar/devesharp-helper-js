...
describe('Obj', () => {
    it('filter', async () => {
        ...
        expect(Obj.filter(object)).toEqual({
            string: 'string',
            object: {
                string: 'string',
            },
    ...
    it('set', async () => {
        ...
        expect(Obj.set(obj, 'simple_object.desk.price', 100)).toEqual({
            value: 2,
            simple_object: {
                desk: {
                    price: 100,
        ...
        expect(Obj.set(obj2, 'simple_object.desk.price', 100)).toEqual({
            value: 2,
            simple_object: {
                desk: {
                    price: 100,
    ...
    it('except', async () => {
        ...
        expect({ value: [1, 2, 4] }).toEqual(Obj.except({ value: [1, 2, 3, 4] }, 'value.2'));

        expect({ value: [2, 4] }).toEqual(Obj.except({ value: [1, 2, 3, 4] }, ['value.2', 'value.0']));
    });
    ...
    it('only', async () => {
        ...
        expect({ array_object: [{ desk: { price: ['A'] } }, { desk: { price: ['A'] } }] }).toEqual(
            Obj.only(obj, 'array_object.desk.price'),
        );
    });
    ...
    it('divide', async () => {
        ...
        expect([['key'], ['value']]).toEqual(Obj.divide({ key: 'value' }));
        expect([
            ['key', 'key2'],
            ['value', { b: 1 }],
    ...
    it('pluck', async () => {
        expect([1, 2, 3]).toEqual(Obj.pluck([{ id: 1 }, { id: 2 }, { id: 3 }], 'id'));
        expect(['Taylor', 'Abigail']).toEqual(
            Obj.pluck(
                [{ developer: { id: 1, name: 'Taylor' } }, { developer: { id: 2, name: 'Abigail' } }],
                'developer.name',
            ),
        );
    });
